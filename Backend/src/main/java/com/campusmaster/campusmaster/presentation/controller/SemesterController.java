...
public class SemesterController {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'TEACHER', 'STUDENT')")
    @Operation(
            summary = "Semestre actuel",
            description = "Récupérer le semestre actuellement actif")
    ...
    @Operation(
            ...
            description = "Créer un nouveau semestre académique (Admin uniquement)")
    ...
    @Operation(
            ...
            description = "Modifier un semestre existant (Admin uniquement)")
    public ResponseEntity<?> updateSemester(
            @PathVariable Long id, @Valid @RequestBody UpdateSemesterRequest request) {
        return semesterRepository
                .findById(id)
                .map(
                        semester -> {
                            ...
                            if (request.getCode() != null
                                    && !semester.getCode().equals(request.getCode())
                                    && semesterRepository.existsByCode(request.getCode())) {
                                return ResponseEntity.badRequest()
                                        .body("Un semestre avec ce code existe déjà");
                            }
                            ...
                            // Si on veut définir ce semestre comme actuel, désactiver tous les
                            // autres
    ...
    @Operation(
            ...
            description = "Supprimer un semestre (Admin uniquement)")
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(
            summary = "Définir comme semestre actuel",
            description = "Définir un semestre comme étant le semestre actuel")
