...
public class TeacherController {
    ...
    @PreAuthorize("hasRole('TEACHER')")
    public ResponseEntity<StudentResponse> valideProfile(
            @AuthenticationPrincipal Teacher teacher,
            @PathVariable Long studentId,
            @PathVariable Boolean isValidated) {
        return new ResponseEntity<>(
                teacherService.validateStudent(teacher, studentId, isValidated), HttpStatus.OK);
    }
}
