...
public class EnrollmentController {
    ...
    @Operation(summary = "S'inscrire à un cours", description = "Permet à un étudiant de s'inscrire à un cours")
    public ResponseEntity<EnrollmentResponse> enrollStudent(
            @PathVariable Long courseId,
            @AuthenticationPrincipal Student currentUser) {
        EnrollmentResponse enrollment = enrollmentService.enrollStudent(courseId, currentUser.getId());
        return ResponseEntity.ok(enrollment);
    }

    @DeleteMapping("/courses/{courseId}/unenroll")
    @PreAuthorize("hasRole('STUDENT')")
    @Operation(summary = "Se désinscrire d'un cours", description = "Permet à un étudiant de se désinscrire d'un cours")
    ...
    @Operation(summary = "Liste des étudiants inscrits", description = "Récupère la liste des étudiants inscrits à un cours")
    ...
    @PreAuthorize("hasRole('STUDENT')")
    @Operation(summary = "Mes inscriptions", description = "Récupère la liste des cours auxquels l'étudiant est inscrit")
    public ResponseEntity<List<EnrollmentResponse>> getMyEnrollments(
            @AuthenticationPrincipal Student currentUser) {
        List<EnrollmentResponse> enrollments = enrollmentService.getEnrollmentsByStudent(currentUser.getId());
        return ResponseEntity.ok(enrollments);
    }

    @GetMapping("/courses/{courseId}/enrollment-status")
    @PreAuthorize("hasRole('STUDENT')")
    @Operation(summary = "Vérifier inscription", description = "Vérifie si l'étudiant est inscrit à un cours")
