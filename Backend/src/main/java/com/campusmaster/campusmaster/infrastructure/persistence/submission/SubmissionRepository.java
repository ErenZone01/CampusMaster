...
public interface SubmissionRepository extends JpaRepository<Submission, Long> {
    ...
    @Query(
            "SELECT s FROM Submission s WHERE s.assignment.teacher.id = :teacherId AND s.grade IS NULL ORDER BY s.submittedAt DESC")
    List<Submission> findPendingSubmissionsByTeacher(@Param("teacherId") Long teacherId);

    @Query(
            "SELECT COUNT(s) FROM Submission s WHERE s.assignment.teacher.id = :teacherId AND s.grade IS NULL")
